...
<div class="col-md-2" style="float:right;"> 
    <a href="{{ url('/internal_medicine/admission/createRequest',[$visit->id]) }}" class="btn btn-warning btn-sm">Admit Patient</a>
</div>
</div>
<br/>
...
<div class="tab">
  ...
  <button class="tablinks" onclick="openCity(event, 'step5')">Entry ward</button>
  ...
  <button class="tablinks" onclick="openCity(event, 'step8')">Lab</button>
  <button class="tablinks" onclick="openCity(event, 'step9')">Lab result</button>
</div>

<!-- Tab content -->

    
<x-forms.post action="{{route('internal_medicine.visit.update')}}">
        ...
        <div class="col-md-12">
            <p>Lipid test</p>   
                        ...
                        <table class="table" id="labs_table">
                                ...
                                <tbody>
                                <tr id="labsTemplate" class="d-none">
                                    <td width="25%">
                                        <x-forms.select-from-pluck name="lab_type[]" value="" :options="['' => '--choose lab type--', 'Lipid test' => 'Lipid test']" />
                                    </td>
                                    <td width="25%">
                                        <x-forms.select-from-pluck name="spec_type[]" value="" :options="['' => '--choose specimen--', 'Blood' => 'Blood', 'Urine' => 'Urine', 'CSF' => 'CSF']" />
                                    </td>
                                    <td width="25%">
                                        <x-forms.select-from-pluck name="priority[]" value="" :options="['' => '-- choose Priority --', 'Urgent' => 'Urgent', 'Routine' => 'Routine']" />
                                    </td>
                                    <td width="25%">
                                            <x-forms.textfield type="text" name="clinical_notes[]" value="" placeholder="clinical notes"/>
                                   </td>
                                </tr>

                                @foreach (($visit->labs) as $lab)

                                    <tr id="labsRow{{ $loop->index + 1}}">

                                    <td width="25%">
                                        <x-forms.select-from-pluck name="lab_type[]" value="{{$lab->lab_type}}" :options="[ 'Lipid test' => 'Lipid test']" required/>
                                    </td>
                                    <td width="25%">
                                        <x-forms.select-from-pluck name="spec_type[]" value="{{$lab->spec_type}}" :options="['Blood' => 'Blood', 'Urine' => 'Urine', 'CSF' => 'CSF']" />
                                    </td>
                                    <td width="25%">
                                        <x-forms.select-from-pluck name="priority[]" value="{{$lab->priority}}" :options="['Urgent' => 'Urgent', 'Routine' => 'Routine']" />
                                    </td>
                                    <td width="25%">
                                        <x-forms.textfield type="text" name="clinical_notes[]" value="{{$lab->clinical_notes}}" placeholder="clinical notes"/>
                                    </td>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <button id="labsAddRow" class="btn btn-success pull-left btn-sm">+ Add Row</button>
                        <button id='labsDeleteRow' class="pull-right btn btn-danger btn-sm">- Delete Row</button>
                    </div>
                </div>
                </div>
                </div>
...
<script>
    ...
    <script>
      ...
      //labs
      $(document).ready(function(){
      let rowCountLabs = {{ old('labsCount', $visit->labs()->count() ) }};
            $("#labsAddRow").click(function(e){
                console.log('labsAddRow clicked ' + rowCountLabs);
                e.preventDefault();
                let new_row_number = rowCountLabs + 1;
                $('#labs_table').append('<tr id="labsRow' + new_row_number + '"></tr>');
                $('#labsRow' + new_row_number).html($('#labsTemplate').html());
                rowCountLabs++;
            });

            $("#labsDeleteRow").click(function(e){
                console.log('labsDeleteRow clicked ' + rowCountLabs);
                e.preventDefault();
                if(rowCountLabs > 0){
                    $("#labsRow" + (rowCountLabs)).remove();
                    rowCountLabs--;
                }
            });
